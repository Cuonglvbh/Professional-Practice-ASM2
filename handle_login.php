...
{
    // kiem tra dang nhap
    // tai customers nhap co ton tai trong database ko?
    $db = connection(); // goi bien ket noi toi database
    // viet cau lenh sql truy van
    $sql = "SELECT * FROM `user_account` WHERE `username` = :account AND `password` = :pass LIMIT 1 ";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btnRegister'])) {
    ...
    // Example: Connect to the database and insert the data into a users table
